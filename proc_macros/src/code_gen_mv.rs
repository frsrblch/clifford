...
impl Algebra {
    pub fn define_mv(self) -> TokenStream {
        ...
        let types = TypeMv::iter(self).filter_map(TypeMv::define);

        let type_impls = TypeMv::iter(self).filter_map(TypeMv::impl_item);
...
impl UnaryOp {
    ...
    fn output(self, ty: TypeMv) -> TypeMv {
        if ty.is_generic() {
            ...
            TypeMv::from_iter(self.products(ty), ty.algebra())
        }
    }
    ...
    fn generics(self, ty: TypeMv) -> syn::Generics {
        ...
        let predicates = ty.algebra().grades().map::<WherePredicate, _>(|grade_in| {
            let ty_out = TypeMv::from_iter(self.products(grade_in), ty.algebra());
            ...
            TypeMv::Multivector(mv) => {
                ...
                let fields = mv.1.grades().map(|grade_out| {
                    let exprs = mv.grades().filter_map(|grade_in| {
                        let ty_out = TypeMv::from_iter(self.products(grade_in), mv.1);
                        match ty_out {
...
impl SumOp {
    pub fn item_impl(self, lhs: TypeMv, rhs: TypeMv) -> Option<ItemImpl> {
        if self.is_std() && lhs.is_scalar() && rhs.is_scalar() {
            return None;
        }
    ...
    fn output(self, lhs: TypeMv, rhs: TypeMv) -> TypeMv {
        TypeMv::from_iter(self.products(lhs, rhs), lhs.algebra())
    }

    fn generics(self, lhs: TypeMv, rhs: TypeMv) -> Generics {
    ...
    fn output_type_item(self, lhs: TypeMv, rhs: TypeMv) -> ImplItem {
    ...
    fn output_generic(self, grade: Grade, lhs: TypeMv, rhs: TypeMv) -> Ident {
    ...
    fn new_output_generic(self, grade: Grade, lhs: TypeMv, rhs: TypeMv) -> Option<Ident> {
        match (lhs.contains(grade), rhs.contains(grade)) {
    ...
    fn fn_item(self, lhs: TypeMv, rhs: TypeMv) -> ImplItem {
    ...
    fn sum_output_expr(self, lhs: TypeMv, rhs: TypeMv) -> Expr {
        ...
        match self.output(lhs, rhs) {
            ...
            TypeMv::Grade(grade) => {
                if is_generic(lhs, rhs) {
                    ...
                    sum_grade_fields_expr(self, grade)
                }
            }
            TypeMv::Multivector(_) => {
                let ty = Multivector::ident();
                let grades = lhs
                    .algebra()
                    .grades()
    ...
    fn sum_grade_sum_expr(self, lhs: TypeMv, rhs: TypeMv, grade: Grade) -> Expr {
...
impl ProductOp {
    ...
    fn last_generics(self, lhs: TypeMv, rhs: TypeMv) -> impl Iterator<Item = Ident> {
        lhs.algebra()
            .grades()
            .map(move |grade| self.last_generic(lhs, rhs, grade))
    }

    fn last_generic(self, lhs: TypeMv, rhs: TypeMv, grade: Grade) -> Ident {
    ...
    fn intermediate_and_sum_types(self, lhs: TypeMv, rhs: TypeMv, grade: Grade) -> usize {
        use std::ops::Mul;
    ...
    fn intermediate_types(self, lhs: TypeMv, rhs: TypeMv, grade: Grade) -> usize {
    ...
    fn output_expr(self, lhs: TypeMv, rhs: TypeMv) -> Expr {
        match self.output(lhs, rhs) {
            TypeMv::Zero(_) => Zero::expr(),
            TypeMv::Grade(grade) => {
                if is_generic(lhs, rhs) {
    ...
    fn grade_fields_expr(self, grade: Grade, lhs: TypeMv, rhs: TypeMv) -> syn::Expr {
        let fields = grade.blades().map(|blade| {
...
mod tests {
    ...
    pub fn scalar_mv_scalar_product() {
        ...
        let actual = ProductOp::Grade(scalar)
            .item_impl(TypeMv::Grade(scalar), TypeMv::Multivector(g2.mv()))
            .unwrap();
    ...
    pub fn scalar_mv_mul() {
        ...
        let actual = ProductOp::Mul
            .item_impl(TypeMv::Grade(scalar), TypeMv::Multivector(algebra.mv()))
            .unwrap();
    ...
    fn mul_scalars_is_none() {
        let algebra = Algebra::new(3, 0, 0);
        let scalar = TypeMv::Grade(algebra.grade(0));
        assert!(ProductOp::Mul.item_impl(scalar, scalar).is_none());
    }
    ...
    fn intermediate_type_test() {
        ...
        assert_eq!(
            1,
            ProductOp::Mul.intermediate_types(mv, TypeMv::Grade(bivector), trivector)
        );
    ...
    fn unary_op_mv_output() {
        ...
        let expected = TypeMv::Multivector({
            let mut mv = Multivector::new(algebra);
